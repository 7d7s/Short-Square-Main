      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 max-w-[1600px] flex flex-col gap-20 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5 w-full">
          {servicesData.map((service, index) => {
            ...
            return (
              <Link key={service.id} href={`/services/${service.slug}`} className={`group relative overflow-hidden rounded-[20px] md:rounded-[24px] h-[450px] md:h-[500px] lg:h-[550px] cursor-pointer border border-white/[0.03] bg-[#050505] block ${colSpan}`}>
                <motion.div 
                  initial={{ opacity: 0, y: 40 }}
                  whileInView={{ opacity: 1, y: 0 }}
